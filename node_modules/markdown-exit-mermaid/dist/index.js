...
}, a = `<div class="mermaid-toolbar">
      <button class="btn copy-code" aria-label="Copy Code" title="Copy Code">${i.copy}</button>
    </div>`, o = `<div class="mermaid-viewer-grid-panel">
        <div class="grid-row">
          <div class="empty-cell"></div>
          <button class="btn up" aria-label="Pan up">${i.up}</button>
          <button class="btn zoom-in" aria-label="Zoom in">${i.zoomIn}</button>
        </div>
        <div class="grid-row">
          <button class="btn left" aria-label="Pan left">${i.left}</button>
          <button class="btn reset" aria-label="Reset view">${i.reset}</button>
          <button class="btn right" aria-label="Pan right">${i.right}</button>
        </div>
        <div class="grid-row">
          <div class="empty-cell"></div>
          <button class="btn down" aria-label="Pan down">${i.down}</button>
          <button class="btn zoom-out" aria-label="Zoom out">${i.zoomOut}</button>
        </div>
      </div>`, s = typeof __dirname < "u" ? __dirname : n.dirname(r(import.meta.url)), c = n.join(s, "assets");
e(c) || (c = n.join(s, "../assets"));
var l = t(n.join(c, "viewer.js"), "utf-8"), u = t(n.join(c, "style.css"), "utf-8"), d = function(e, t = {}) {
	let n = t.theme || "default", r = t.js_url || "https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.min.js", i = e.renderer.rules.fence;
	e.renderer.rules.fence = (e, s, c, d, p) => {
		let m = e[s];
		if (!m) return "";
		if ((m.info || "").trim().split(/\s+/)[0] !== "mermaid") return i ? i(e, s, c, d, p) : p.renderToken(e, s, c);
		let h = f(m.content) || "", g = `mermaid-${Math.random().toString(36).slice(2)}`, _ = t.css_url ? `<link rel="stylesheet" href="${t.css_url}">` : `<style>${u}</style>`, v = "";
		return v = t.viewer_js_url ? `<script src="${t.viewer_js_url}"><\/script><script>window.initMermaidDiagram('${g}', '${r}', '${n}')<\/script>` : `<script>
        if (!window.initMermaidDiagram) {
          ${l}
        }
        window.initMermaidDiagram('${g}', '${r}', '${n}', ${JSON.stringify(t.theme_variables || {})});
      <\/script>`, `
<div id="${g}" class="mermaid-container">
  ${t.html_only ? "" : _}
  <div class="mermaid-wrapper">
    ...
    <div class="mermaid-view-container">
      ...
      <pre class="mermaid-content">
         <code class="mermaid-code" style="display: none;">${h}</code>
      </pre>
    </div>
  </div>
  ${t.html_only ? "" : v}
